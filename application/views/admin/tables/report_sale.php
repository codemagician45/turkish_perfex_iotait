...
$sale_phases  = $this->ci->sale_model->get_sale_phases(); 
$sale_phasesIds = [];

foreach ($sale_phases as $s_phase) {
    if ($this->ci->input->post('s_phase_' . $s_phase['order_no'])) {
        array_push($sale_phasesIds, $s_phase['order_no']);
    }
}
if (count($sale_phasesIds) > 0) {
    array_push($filter, 'AND tblestimates.sale_phase_id IN (' . implode(', ', $sale_phasesIds) . ')');
}
