...
class Production_model extends App_Model
{
    ...
    public function produced_qty($produced_qty_data)
    {
        print_r($produced_qty_data); exit();
        $produced_qty_data['userid'] = get_staff_user_id();
        $this->db->where('id',$produced_qty_data['machine_id']);
        $machine = $this->db->get(db_prefix().'machines_list')->row();
        $take_from = $machine->take_from;
        $export_to = $machine->export_to;
        $produced_qty_arr = [];
        $produced_qty_arr[] = array(
            'shift' => 'day',
            'name' => $produced_qty_data['day_name'],
            'working_time' => $produced_qty_data['day_working_time'],
            'produced_quantity' => $produced_qty_data['day_produced_quantity'],
            'waste_production_quantity' => $produced_qty_data['day_waste_production_quantity']
        );
        $produced_qty_arr[] = array(
            'shift' => 'night',
            'name' => $produced_qty_data['night_name'],
            'working_time' => $produced_qty_data['night_working_time'],
            'produced_quantity' => $produced_qty_data['night_produced_quantity'],
            'waste_production_quantity' => $produced_qty_data['night_waste_production_quantity']
        );
        foreach ($produced_qty_arr as $data){
