...
$(function () {
    ...
    var plan_recipe_calendar_production_selector = $('#plan_recipe_calendar_production');

    if (plan_recipe_calendar_production_selector.length > 0) {
        validate_calendar_form();
        var calendar_settings = {
            themeSystem: 'bootstrap3',
            customButtons: {},
            ...
            eventRender: function (event, element) {
                ...
                if (!event.url) {
                    element.click(function () { 
                        // view_plan_event_production(event.eventid); 
                    });
                }
            },
        };
        ...
        // Init calendar
        plan_recipe_calendar_production_selector.fullCalendar(calendar_settings);
        var new_event = get_url_param('new_event');
        if (new_event) {
            $("input[name='start'].datetimepicker").val(get_url_param('date'));
            $('#planNewEventModal').modal('show');
    ...
    function view_plan_event_production(id) {
        if (typeof (id) == 'undefined') { return; }
        $.post(admin_url + 'invoices/view_plan_event/' + id).done(function (response) {
            $('#event').html(response);
            console.log($('#viewPlanEvent'),$('#viewPlanEventProduction'))
            $('#viewPlanEventProduction').modal('show');
            var eventData = {
                        start:'',
                        end:''
                    };
    ...
    var installation_calendar_selector = $('#installation_calendar');
    if (installation_calendar_selector.length > 0) {
        validate_calendar_form();
        var calendar_settings = {
            themeSystem: 'bootstrap3',
            customButtons: {},
            ...
            eventSources: [{
                url: admin_url + 'utilities/get_installation_calendar_data',
                data: function () {
                    var params = {};
                    $('#calendar_filters').find('input:checkbox:checked').map(function () {
                        params[$(this).attr('name')] = true;
            ...
            eventRender: function (event, element) {
                ...
                element.attr('data-toggle', 'tooltip');
                if (!event.url) {
                    element.click(function () { view_installation_event(event.eventid); });
                }
            },
        };
        ...
        // Init calendar
        installation_calendar_selector.fullCalendar(calendar_settings);
        var new_event = get_url_param('new_event');
        if (new_event) {
            $("input[name='start'].datetimepicker").val(get_url_param('date'));
    ...
    var installation_calendar_production_selector = $('#installation_calendar_production');
    if (installation_calendar_production_selector.length > 0) {
        validate_calendar_form();
        var calendar_settings = {
            themeSystem: 'bootstrap3',
            customButtons: {},
            ...
            eventRender: function (event, element) {
                ...
                if (!event.url) {
                    element.click(function () { 
                        // view_installation_event(event.eventid); 
                    });
                }
            },
        };
        ...
        // Init calendar
        installation_calendar_production_selector.fullCalendar(calendar_settings);
        var new_event = get_url_param('new_event');
        if (new_event) {
            $("input[name='start'].datetimepicker").val(get_url_param('date'));
            $('#installationNewModal').modal('show');
    ...
    function view_installation_event(id)
    {
        if (typeof (id) == 'undefined') { return; }
        $.post(admin_url + 'invoices/view_installation_event/' + id).done(function (response) {
            $('#event').html(response);
            $('#viewInstallationEvent').modal('show');
            ...
            $('#edit_installation_events').fullCalendar(calendar_settings);
            validate_calendar_form();
        });
    }

    var installation_view_calendar_selector = $('#installation_view_calendar');
    if (installation_view_calendar_selector.length > 0) {
        validate_calendar_form();
        var calendar_settings = {
            themeSystem: 'bootstrap3',
            customButtons: {},
        ...
        // Init calendar
        installation_view_calendar_selector.fullCalendar(calendar_settings);
        // var new_event = get_url_param('new_event');
        // if (new_event) {
        //     $("input[name='start'].datetimepicker").val(get_url_param('date'));
        //     $('#installationNewModal').modal('show');
...
function init_pending_estimate(id) {
    load_small_table_item(id, '#estimate', 'estimateid', 'estimates/get_pending_estimate_data_ajax', '.table-p_sale_order');
}
...
function delete_installation_event(id)
{
    if (confirm_delete()) {
        requestGetJSON('utilities/delete_installation_event/' + id).done(function (response) {
            if (response.success === true || response.success == 'true') { window.location.reload(); }
        });
    }
}
